...
class GPT(nn.Module):
    ...
    def forward(self, x):
        ...
        dvs_fea = x[1]   # ir_fea (tensor): dim:(B, C, H, W)
        print("GPT forward")
        print(rgb_fea.shape)
        print(dvs_fea.shape)
        assert rgb_fea.shape[0] == dvs_fea.shape[0]
        ...
        dvs_fea_flat = dvs_fea.view(bs, c, -1)  # flatten the feature
        token_embeddings = torch.cat([rgb_fea_flat, dvs_fea_flat], dim=2)  # concat
        ...
        dvs_fea_out = x[:, 1, :, :, :].contiguous().view(bs, self.n_embd, self.vert_anchors, self.horz_anchors)

        # -------------------------------------------------------------------------
        # Interpolate (or Upsample)
        # -------------------------------------------------------------------------
        rgb_fea_out = F.interpolate(rgb_fea_out, size=([h, w]), mode='bilinear')
        dvs_fea_out = F.interpolate(dvs_fea_out, size=([h, w]), mode='bilinear')
        print("GPT forward out")
        print(rgb_fea_out.shape)
        print(dvs_fea_out.shape)

        return rgb_fea_out, dvs_fea_out